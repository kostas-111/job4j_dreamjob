...
public class Sql2oCandidateRepository implements CandidateRepository {
    ...
    public Candidate save(Candidate candidate) {
        try (var connection = sql2o.open()) {
            var sql = """
                      INSERT INTO candidates(candidate_name, description, creation_date, city_id, file_id)
    ...
    public boolean deleteById(int id) {
        try (var connection = sql2o.open()) {
            ...
            int deletedRow = query.executeUpdate().getResult();
            return deletedRow > 0;
        }
    }
    ...
    public boolean update(Candidate candidate) {
        try (var connection = sql2o.open()) {
            var sql = """
                    UPDATE candidates
                    SET candidate_name = :name, description = :description, creation_date = :creationDate,
